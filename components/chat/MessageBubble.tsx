...
function detectInstagramUrl(text: string): string | null {
  const match = text.match(
    /https?:\/\/(www\.)?instagram\.com\/(reel|p|tv)\/[A-Za-z0-9_-]+\/?(\?[A-Za-z0-9&=_%.-]*)?/
  );
...
export function MessageBubble({ message, isOwn, userName, onReply, onReact, onDelete, onScrollToMessage, highlighted }: Props) {
  ...
  const gifUrl = !isDeleted && message.type === "text" ? detectGifUrl(message.content) : null;
  const igUrl = !isDeleted && message.type === "text" && !gifUrl ? detectInstagramUrl(message.content) : null;
